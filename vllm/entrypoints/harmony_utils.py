    ...
    if "type" not in response_msg or response_msg["type"] == "message":
        ...
        for prev_response in reversed(prev_responses):
            ...
            is_function_call = isinstance(prev_response, ResponseFunctionToolCall) or (
                ...
                and prev_response.get("type") == "function_call"
            )

            if is_function_call:
                # Get call_id from either Pydantic object or dict
                prev_call_id = (
                    prev_response.call_id
                    if hasattr(prev_response, "call_id")
                    else prev_response.get("call_id")
                )
        ...
        function_name = (
            call_response.name
            if hasattr(call_response, "name")
            else call_response.get("name")
        )
